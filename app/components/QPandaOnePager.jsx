...
<OverlayFX tethers={tethers} glitchAt={glitch ? pandaPos : null} />

<AnimatePresence>
  {actionLabel && (
    <motion.div
      className="pointer-events-none fixed z-30 rounded-full border border-cyan-400/40 bg-slate-900/80 px-3 py-1 text-xs text-cyan-200 backdrop-blur"
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -8 }}
      transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
      style={{ left: pandaPos.x + 24, top: pandaPos.y - 28 }}
    >
      {actionLabel}
    </motion.div>
  )}
